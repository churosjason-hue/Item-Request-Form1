...
const APPROVER_TYPES = [
  { value: 'role', label: 'By Role (System Default)' },
  { value: 'user', label: 'Specific User' },
  { value: 'department', label: 'Department' },
  { value: 'department_approver', label: 'Department Approver' },
  { value: 'custom_matrix_role', label: 'Custom Matrix Role' },
...
export default function WorkflowSettings() {
  ...
  const modalContentRef = useRef(null); // Ref for auto-scrolling
  ...
  const handleDragStart = (e, index) => {
    ...
    // Usually handled by browser, but we can track state if specialized styling needed
  };
  ...
  const handleDragOver = (e, index) => {
    ...
    if (modalContentRef.current) {
      const { top, bottom, height } = modalContentRef.current.getBoundingClientRect();
      const clientY = e.clientY;
      const scrollThreshold = 100; // Distance from edge to start scrolling
      const scrollSpeed = 10; // Pixels per frame/event

      if (clientY < top + scrollThreshold) {
        // Scroll Up
        modalContentRef.current.scrollTop -= scrollSpeed;
  ...
  const handleMoveStep = (index, direction) => {
    ...
    if (index + direction < 0 || index + direction >= formData.steps.length) return;
  ...
  const handleSubmit = async () => {
    try {
      ...
      for (const step of formData.steps) {
        ...
        if (step.approver_type === 'custom_matrix_role' && !step.approver_role) {
          alert(`Step ${step.step_order}: Please specify the custom matrix role name`);
          return;
        }
      ...
      // Pre-process formData
      const submissionData = { ...formData };
      if (!submissionData.department_id) {
        submissionData.department_id = null; // Important to send null if empty so backend sets Global default
      }
      if (submissionData.department_id) {
        submissionData.is_default = false; // Department workflows can't be global defaults
      }

      setLoading(true);

      if (editingWorkflow) {
        await workflowsAPI.update(editingWorkflow.id, submissionData);
        alert('Workflow updated successfully!');
      } else {
        await workflowsAPI.create(submissionData);
  ...
  const handleThemeToggle = async () => {
    try {
      const newValue = !helloKittyTheme;
      setHelloKittyTheme(newValue); // Optimistic update
      await settingsAPI.update('service_desk_theme_enabled', newValue);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden mb-8">
          ...
          <div className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-md font-medium text-gray-900">Service Desk Appearance</h3>
                <p className="text-sm text-gray-500">Enable the Hello Kitty theme for Service Desk users.</p>
              </div>
              <button
                onClick={handleThemeToggle}
                className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${helloKittyTheme ? 'bg-pink-500' : 'bg-gray-200'}`}
              >
                <span className="sr-only">Use setting</span>
                <span
                  aria-hidden="true"
                  className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 ${helloKittyTheme ? 'translate-x-5' : 'translate-x-0'}`}
                />
              </button>
            </div>
          </div>
        ...
        {showCreateModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div ref={modalContentRef} className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {formData.steps.map((step, index) => (
                      <div
                        key={index}
                        className="border border-gray-300 rounded-lg p-4 bg-gray-50 cursor-move transition-shadow hover:shadow-md"
                        draggable
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center space-x-2">
                            <div className="flex items-center space-x-1 mr-2 border-r border-gray-300 pr-2">
                              <button
                                type="button"
                              ...
                              <button
                                ...
                                className={`p-1 rounded ${index === formData.steps.length - 1 ? 'text-gray-300' : 'text-gray-600 hover:bg-gray-200'}`}
                                title="Move Down"
                              >
