...
export default function ServiceVehicleRequestForm() {
  ...
  const isEditing = location.pathname.endsWith("/edit") || location.pathname.includes("/edit/");
  ...
  const handleChange = (e) => {
    ...
    if (name === "has_valid_license") {
      if (value === "true") {
        updates.driver_name = formData.requestor_name;
      } else {
        updates.driver_name = "";
      }
    }
  ...
  const handleSubmit = (arg = null) => {
    ...
    const cleanedFormData = { ...formData, status: formData.status === "submitted" ? "submitted" : "draft" }; // Preserve submitted status or save as draft first
    ...
    setSubmitArgs(justificationOverride);
    setShowSubmitConfirmModal(true);
  };

  const processSubmit = async () => {
    const justificationOverride = typeof submitArgs === 'string' ? submitArgs : null;
    ...
    const cleanedFormData = { ...formData, status: formData.status === "submitted" ? "submitted" : "draft" };
    if (justificationOverride) {
      cleanedFormData.urgency_justification = justificationOverride;
    }
  ...
  const handleAssignVerifier = async (verifierId, reason) => {
    try {
      setLoading(true);
      await serviceVehicleRequestsAPI.assignVerifier(id, { verifier_id: verifierId, reason });
  ...
  const handleCancelRequest = async (reason) => {
    try {
      setLoading(true);
      await serviceVehicleRequestsAPI.cancel(id, { reason });
      toastSuccess('Request cancelled successfully');
      setTimeout(() => navigate('/dashboard'), 1500);
    } catch (err) {
      toastError(err.response?.data?.message || err.message || "Failed to cancel request");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDecline = async () => {
    setActionModalState({
      ...
      options: [
        ...
        { label: "No available vehicle (already booked, under maintenance, or out of service)", value: "No available vehicle (already booked, under maintenance, or out of service)" },
        { label: "No available driver (on leave, assigned to higher-priority trips)", value: "No available driver (on leave, assigned to higher-priority trips)" },
        { label: "Short notice requests that don't meet the required lead time", value: "Short notice requests that don't meet the required lead time" },
        { label: "Schedule conflict with pre-approved or priority trips", value: "Schedule conflict with pre-approved or priority trips" },
        { label: "Request does not meet company policy (non-work related, personal errands)", value: "Request does not meet company policy (non-work related, personal errands)" },
        { label: "Trip duration or route not justified or approved based on business need", value: "Trip duration or route not justified or approved based on business need" },
        { label: "Route considered unsafe or high-risk (weather, security)", value: "Route considered unsafe or high-risk (weather, security)" },
        { label: "Vehicle not suitable for the purpose (capacity)", value: "Vehicle not suitable for the purpose (capacity)" },
        { label: "Lower priority compared to operational-critical trips", value: "Lower priority compared to operational-critical trips" },
        { label: "Alternative arrangements recommended (public transport, ride-hailing, carpool)", value: "Alternative arrangements recommended (public transport, ride-hailing, carpool)" }
      ],
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 py-8 transition-colors duration-200">
      ...
      >
        <div className="p-8 md:p-12 print:!p-0" style={{ minHeight: "auto" }}>
          {/* Header Section */}
          <div className="border-b-2 border-gray-800 pb-4 mb-6 print:mb-2 print:pb-2">
            <div className="flex items-center justify-between mb-4 print:mb-2 text-gray-900">
              <div className="flex items-center space-x-3">
                <img src={STC_LOGO} alt="STC Logo" className="h-16 w-auto print:h-12" />
                <div>
            ...
            <div className="mb-6 p-4 bg-purple-50 border-2 border-purple-500 rounded-md no-print">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  ...
                  {formData.verifier_reason && (
                    <div className="text-sm text-purple-700 mt-1">
                      <span className="font-semibold">Reason:</span> {formData.verifier_reason}
                    </div>
                  )}
          ...
          <form className="space-y-6">
            ...
            <div className="border border-gray-400 p-4 mb-6 print:mb-3 print:p-2 print:break-inside-avoid">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:grid-cols-4 print:gap-2">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1 print:text-[10px] print:mb-0">
                  ...
                  >
                    <input
                      ...
                      {...getInputProps({
                        ...
                        className:
                          "w-full bg-transparent border-0 focus:outline-none text-sm text-gray-900 placeholder-gray-500 print:text-xs",
                        disabled: loading,
                      })}
                    />
                  </div>
            ...
            <div className="border border-gray-400 p-4 mb-6 print:mb-3 print:p-2 print:break-inside-avoid">
              ...
              <div className="print:!grid print:!grid-cols-12 print:!gap-2">
                {formData.urgency_justification && (
                  <div className="mb-3 md:mb-4 flex flex-col print:!col-span-12">
                    ...
                    <textarea
                      ...
                      readOnly
                      className="w-full border border-red-300 bg-red-50 px-2 py-1 text-xs focus:outline-none text-gray-900"
                      rows="2"
                    ></textarea>
                  </div>
                )}
                ...
                {/* Purpose */}
                <div className="mb-3 md:mb-4 flex flex-col print:!col-span-3">
                  ...
                  <textarea
                    ...
                    {...getInputProps({
                      onChange: handleChange,
                      className:
                        "w-full border border-black px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white text-gray-900 print:!border-0 print:!resize-none print:!px-0 print:!m-0",
                ...
                {/* Request Type */}
                <div className="mb-3 md:mb-4 flex flex-col print:!col-span-4">
                  <label className="text-xs font-semibold block mb-1 text-gray-700">
                  ...
                  <select
                    ...
                    {...getInputProps({
                      onChange: handleChange,
                      className:
                        "w-full border border-black px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white text-gray-900 print:!border-0 print:!appearance-none print:!px-0 print:!m-0",
                      disabled: loading || isReadOnly,
                    })}
                  >
                ...
                {/* Travel Dates */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 md:gap-x-8 md:gap-y-2 mb-3 md:mb-4 print:!col-span-5 print:!grid-cols-2 print:!gap-2 print:!mb-0">
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      {...getInputProps({
                        onChange: handleChange,
                        className:
                          "border-b border-black px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white text-gray-900 print:!border-0 print:!px-0 print:!m-0",
                        disabled: loading || isReadOnly,
                      })}
                    />
            ...
            {formData.request_type === "car_only" && (
              <div className="border border-gray-400 p-4 mb-6 print:mb-3 print:p-2 print:break-inside-avoid">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-700 mb-1">
                      Driver's Name <span className="text-red-600">*</span>
                    </label>
                    <div
                      className={`border-b-2 pb-1 print:border-b-0 ${errors.driver_name
                        ? "border-red-500"
                        : "border-gray-400"
                        }`}
                    >
                      <input
                        ...
                        {...getInputProps({
                          ...
                          disabled: loading || formData.has_valid_license === "true",
                          readOnly: formData.has_valid_license === "true",
                        })}
                      />
                    </div>
                    {errors.driver_name && (
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t-2 border-gray-400 mt-8 no-print">
              ...
              {/* Cancel Request (Requestor only, for submitted or dept_approved) */}
              {(isViewing || isEditing) &&
                user?.id === formData.requested_by &&
                ['submitted', 'department_approved'].includes(formData.status) && (
                  <button
                    type="button"
                    disabled={loading}
                    onClick={() =>
                      setActionModalState({
                        ...
                        message: 'Please provide a reason for cancelling this request. This action cannot be undone.',
                    ...
                    className="flex items-center px-6 py-2 bg-red-100 text-red-700 border border-red-300 rounded hover:bg-red-200 disabled:opacity-50 text-sm font-semibold"
                  >
                    <XCircle className="h-4 w-4 mr-2" />
                    Cancel Request
                  </button>
      ...
      {showJustificationModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-lg w-full print:!shadow-none print:!p-0 print:!m-0">
